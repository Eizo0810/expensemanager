...
$sharedMaven = Join-Path $projectRoot "..\inventorymanager\.tools\apache-maven-3.9.16\bin\mvn.cmd"
$mavenUserHome = Join-Path $projectRoot ".tools\m2"
$sharedMavenUserHome = Join-Path $projectRoot "..\inventorymanager\.tools\m2"
...
if (Test-Path $sharedMaven) {
    $env:MAVEN_USER_HOME = $sharedMavenUserHome
    $sharedMavenArgs = @("-Dmaven.repo.local=$sharedMavenUserHome\repository", "test")
    & $sharedMaven @sharedMavenArgs
    exit $LASTEXITCODE
}
